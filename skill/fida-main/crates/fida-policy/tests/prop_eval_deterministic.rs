// Feature: fida-mvp, Property 1: Evaluation is deterministic and total
//
// Property 1: for any compiled policy and any action, `evaluate(policy, action)`
// is deterministic (repeated calls return an identical `DecisionResult`) and
// total (it always returns exactly one result — it never panics and never
// omits a decision).
//
//
// Determinism is asserted by calling `evaluate` twice on the same inputs and
// comparing the two `DecisionResult`s (which derive `PartialEq`). Totality is
// demonstrated structurally: `evaluate` returns `DecisionResult` by value, so
// it cannot omit a result; exercising it across a broad, randomized input space
// (every action kind, plus weird paths/hosts/tool names) without panicking
// shows it is total over realistic inputs.
...
use fida_action::{
    Action, ActionKind, ActionPayload, Actor, Decision, Finding, Mode, NetTarget, Protocol,
};
...
//
// Mirrors the strategy used by the round-trip property test; copied locally so
// this file remains self-contained (no shared test module to clash with
// sibling prop_eval_*.rs files).
// ---------------------------------------------------------------------------
...
// exercise the evaluator broadly (totality).
// ---------------------------------------------------------------------------
...
/// An argv token that may contain odd characters, but is always non-empty.
...
/// A path that may be empty, absolute, relative, or contain glob-ish chars.
...
/// A host: hostnames, IP literals (including link-local/metadata), edge cases.
...
fn action() -> impl Strategy<Value = Action> {
    ...
    // file.read / file.write / file.delete all carry a File payload.
    let file = (
        prop_oneof![
...
proptest! {
    ...
    // Feature: fida-mvp, Property 1: Evaluation is deterministic and total
    #[test]
    fn evaluation_is_deterministic_and_total(policy in policy_file(), action in action()) {
        let compiled = load_via_yaml(&policy);

        // Totality: each call returns exactly one DecisionResult (by type) and
        // does not panic across the broad randomized input space.
        let first = evaluate(&compiled, &action);
        let second = evaluate(&compiled, &action);

        // Determinism: repeated evaluation of identical inputs is identical.
        prop_assert_eq!(first, second, "evaluate was not deterministic for the same inputs");
    }
}
