...
export const AuthModule: React.FC<AuthModuleProps> = ({ onLoginSuccess, onShowToast }) => {
  ...
  // Staff form state
  const [staffEmail, setStaffEmail] = useState('admin@smkn1jakarta.sch.id');
  const [staffPassword, setStaffPassword] = useState('password123');
  const [staffRoleSelect, setStaffRoleSelect] = useState<UserRole>('admin_tu');
  const [showStaffPassword, setShowStaffPassword] = useState(false);

  // Student form state
  const [studentNisn, setStudentNisn] = useState('0071829384');
  const [studentPassword, setStudentPassword] = useState('siswa2026');
  const [showStudentPassword, setShowStudentPassword] = useState(false);

  // Parent form state
  const [parentNisn, setParentNisn] = useState('0071829384');
  const [parentDob, setParentDob] = useState('2008-04-15');
  ...
  const [spmbRegNo, setSpmbRegNo] = useState('SPMB-2026-0089');
  const [spmbPassword, setSpmbPassword] = useState('spmb2026');
  ...
  const handleStaffSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { role } = await AuthService.login(staffEmail, staffPassword);
      addAuditLog('AUTH_LOGIN_STAFF', `Staff login via API sebagai ${role} (${staffEmail})`, staffEmail, role);
      onShowToast('Login Berhasil', `Selamat datang di Portal ${role.toUpperCase().replace('_', ' ')}`, 'success');
      onLoginSuccess(role, staffEmail);
    } catch {
      addAuditLog('AUTH_LOGIN_STAFF', `Staff login offline/mock sebagai ${staffRoleSelect} (${staffEmail})`, staffEmail, staffRoleSelect);
      onShowToast('Mode Offline (API belum jalan)', 'Backend belum merespons — memakai data mock lokal.', 'warning');
      onLoginSuccess(staffRoleSelect, staffEmail);
    }
  };
  ...
  const handleStudentSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!studentNisn || !studentPassword) {
      onShowToast('Validasi Gagal', 'NISN dan password wajib diisi.', 'error');
      return;
    }
    try {
      const { role } = await AuthService.login(studentNisn, studentPassword);
      addAuditLog('AUTH_LOGIN_SISWA', `Siswa login via API dengan NISN ${studentNisn}`, studentNisn, role);
      onShowToast('Login Siswa Berhasil', 'Memasuki Dashboard Pembelajaran & LMS Siswa', 'success');
      onLoginSuccess(role, studentNisn);
    } catch {
      addAuditLog('AUTH_LOGIN_SISWA', `Siswa login offline/mock dengan NISN ${studentNisn}`, studentNisn, 'siswa');
      onShowToast('Mode Offline (API belum jalan)', 'Memakai data mock lokal.', 'warning');
      onLoginSuccess('siswa', studentNisn);
    }
  };
  ...
  const handleParentSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!parentNisn || !parentDob) {
      onShowToast('Validasi Gagal', 'NISN Anak dan Tanggal Lahir wajib diisi.', 'error');
      return;
    }
    if (useOtp) {
      try {
        const ok = await AuthService.verifyOtp(parentNisn, otpCode);
        if (!ok) {
        ...
        if (otpCode !== '589214') {
          onShowToast('OTP Tidak Valid', 'Kode OTP 6-digit salah atau kedaluwarsa. Gunakan simulasi kode: 589214', 'error');
          return;
        }
      }
    }
    addAuditLog('AUTH_LOGIN_ORANG_TUA', `Orang Tua login untuk NISN anak ${parentNisn}`, `Ortu-${parentNisn}`, 'orang_tua');
    onShowToast('Login Orang Tua Berhasil', 'Memasuki Portal Monitoring Akademik & Kehadiran Anak', 'success');
    onLoginSuccess('orang_tua', parentNisn);
  };
  ...
  const handleSendOtp = async () => {
    try {
      ...
      setOtpSent(true);
      setOtpCode(debugCode ?? '589214');
      onShowToast('OTP Terkirim', `Kode OTP dikirim via WhatsApp (debug: ${debugCode ?? '589214'})`, 'info');
    } catch {
      setOtpSent(true);
      setOtpCode('589214');
      onShowToast('OTP Terkirim', 'Kode OTP 6-digit dikirim via WhatsApp ke 0812****456: [589214]', 'info');
    }
  };

  // Handle SPMB Candidate Login
  const handleSpmbSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!spmbRegNo || !spmbPassword) {
      onShowToast('Validasi Gagal', 'Nomor Pendaftaran dan password wajib diisi.', 'error');
      return;
    }
    try {
      const { role } = await AuthService.login(spmbRegNo, spmbPassword);
      addAuditLog('AUTH_LOGIN_SPMB', `Calon siswa login via API (${spmbRegNo})`, spmbRegNo, role);
      onShowToast('Login Calon Siswa Berhasil', 'Memasuki Portal Pendaftaran Murid Baru (SPMB)', 'success');
      onLoginSuccess(role, spmbRegNo);
    } catch {
      addAuditLog('AUTH_LOGIN_SPMB', `Calon siswa login offline/mock (${spmbRegNo})`, spmbRegNo, 'calon_siswa');
      onShowToast('Mode Offline (API belum jalan)', 'Memakai data mock lokal.', 'warning');
      onLoginSuccess('calon_siswa', spmbRegNo);
    }
  };
            ...
            <form onSubmit={handleStaffSubmit} className="space-y-4" data-testid="form-staff-login">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setStaffRoleSelect(e.target.value as UserRole)}
                ...
                >
                  <option value="admin_tu">Admin / Tata Usaha (TU)</option>
                  <option value="guru">Guru Pengampu Mata Pelajaran</option>
                  <option value="bendahara">Bendahara Keuangan Sekolah</option>
                  <option value="operator">Operator Sistem & Presensi</option>
              ...
              <button
                type="submit"
                className="w-full py-2.5 px-4 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-teal-600/20 flex items-center justify-center gap-2 mt-2"
                ...
                <div className="p-3 bg-teal-50/50 rounded-xl border border-teal-200 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    {!otpSent ? (
                      ...
                      <span className="text-[11px] text-teal-700 font-mono">OTP Terkirim (Demo: 589214)</span>
                    )}
                  </div>
                  <input
                    type="text"
