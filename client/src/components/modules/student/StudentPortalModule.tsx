...
import { generateSvgQrMatrix } from '../../../utils/helpers';
...
export const StudentPortalModule: React.FC<StudentPortalModuleProps> = ({
  ...
  // Filter assignments
  const studentAssignments = assignments.filter((a) => a.kelas === student.kelas);
      ...
      <div className="bg-gradient-to-r from-teal-700 via-teal-800 to-slate-900 text-white p-6 rounded-2xl shadow-lg relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-teal-200/80">{student.kelas}</span>
              </div>
              <h2 className="text-lg font-bold text-white mt-1">{student.name}</h2>
              <p className="text-xs text-teal-100/80">{student.jurusan}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="bg-white/10 backdrop-blur-md px-3.5 py-2 rounded-xl border border-white/20 text-center">
              ...
              <p className="text-lg font-black text-white">{avgGrade}</p>
            </div>
            <div className="bg-white/10 backdrop-blur-md px-3.5 py-2 rounded-xl border border-white/20 text-center">
              <p className="text-[10px] text-teal-200 uppercase font-semibold">Peringkat</p>
              <p className="text-lg font-black text-white">2 / 36</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-teal-600/50 flex flex-wrap items-center justify-between gap-3 text-xs">
          ...
          <button
            ...
            className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl shadow-md transition-all flex items-center gap-1.5 text-xs"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-400 font-medium">Kurikulum Merdeka</span>
            </div>
              ...
              {materials.map((mat) => (
                ...
                >
                  ...
                  <div className="pt-2 border-t border-slate-200 flex items-center justify-between text-[11px]">
                    ...
                    <button
                      onClick={() => onShowToast('Unduh Materi', `Mengunduh berkas ${mat.title} (${mat.fileSize})`, 'info')}
      ...
      {activeBottomNav === 'tugas' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            ...
            <span className="text-xs text-slate-500 font-medium">Kelas {student.kelas}</span>
          </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-100 text-xs">
                  ...
                  {asn.status === 'Belum Dikerjakan' ? (
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-xs font-bold transition-colors shadow-xs flex items-center gap-1.5"
      ...
      {activeBottomNav === 'jadwal' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-2">
            ...
            <span>Jadwal Pelajaran KBM Mingguan ({student.kelas})</span>
          </h3>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-6">
          <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-teal-700 font-bold">NISN: {student.nisn}</p>
              <p className="text-xs text-slate-500 mt-0.5">{student.kelas} — {student.jurusan}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
              ...
              <p className="font-bold text-slate-800">{student.birthPlace}, {student.birthDate}</p>
            </div>
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
              <span className="text-slate-400 font-semibold">No. Telepon Orang Tua</span>
              <p className="font-mono font-bold text-slate-800">{student.parentPhone}</p>
            </div>
          </div>

          <button
            onClick={() => setIsKtsModalOpen(true)}
            className="w-full py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-xs font-bold shadow-xs transition-colors flex items-center justify-center gap-2"
          >
          ...
          <form onSubmit={handleAssignmentSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                Nama Berkas Tugas (ZIP, PDF, DOCX)
              </label>
              <input
                type="text"
